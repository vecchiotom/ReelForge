...
- **Anthropic (Claude) provider kind** (`InferenceProviderKind.Anthropic`, official `Anthropic` NuGet SDK → `AsIChatClient`) serves `Chat` and `Vision` only — `Transcription` is rejected with a 400 at the API boundary since Anthropic ships no speech-to-text endpoint. Deliberately a **model backend, not the [Claude Agent SDK](https://code.claude.com/docs/en/agent-sdk/overview)**: ReelForge's own harness (`ReelForgeAgentBase` + `AgentStepExecutor` + `ToolGroupCatalog` + the rooms) stays the harness, so the whole integration is one arm of `ChatClientFactory.Build` and no agent changed. The stored key selects its own transport — blank defers to the SDK's own env/profile resolution (nothing persisted), `sk-ant-oat…` goes out as `Authorization: Bearer`, anything else as `x-api-key`. Because `ReelForgeAgentBase.BuildChatOptions` attaches an **OpenAI-SDK-typed** raw representation for `ReasoningEffort` and is built before the per-run provider is known, the Anthropic client is wrapped in `OpenAIRawOptionsStrippingChatClient`, which drops that factory (cloning, never mutating the agent's reused instance) and leaves both OpenAI paths byte-identical; a per-agent reasoning effort is consequently not forwarded to Claude. **Credential boundary:** a Pro/Max subscription token is for individual use by its owner only — a deployed ReelForge serving other people must use a Console API key or Bedrock/Vertex/Foundry. See [`docs/anthropic-provider.md`](docs/anthropic-provider.md).
