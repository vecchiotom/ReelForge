...
│       ├── Agents/                               # All 20 workflow agents (11 original + VideoStoryEditor + MotionGraphicsPlanner + VideoReviewAgent + MusicSupervisor + VideoEditDirector + MotionGraphicsDirector + Colorist + ColorGradeDirector + SoundDesigner)
│       │   ├── Analysis/                         # 5 code analysis agents
│       │   ├── Translation/                      # Remotion + Animation agents
│       │   ├── Production/                       # Director, Scriptwriter, Author, VideoStoryEditor, MotionGraphicsPlanner, MusicSupervisor, Colorist + the room directors
...
- **Sandbox file tools** are scoped through `ToolGroupCatalog` + `AgentToolProvider` (kept in lockstep by `ToolScopingDriftGuardTests`). `ToolGroup.SandboxAuthoring` holds `WriteSandboxFile` plus the targeted editors `EditSandboxFile`/`ApplySandboxFileEdits`; `ToolGroup.SandboxBrowse` holds `ReadSandboxFile` plus `ReadSandboxFileLines`/`GetSandboxFileOutline`. Agents are prompted to prefer a targeted edit over re-emitting a whole file — the latter wastes output tokens and reliably drops code the model meant to keep. Edits are all-or-nothing across a multi-hunk call, ordinal (never regex), tolerant of a mismatched line ending, and return `{"ok": false, "error": ...}` rather than throwing, so the model self-corrects on its next turn instead of burning a step retry. See [`docs/workflow-execution-optimization.md`](docs/workflow-execution-optimization.md).
- **Built-in agent system prompts are duplicated** between each agent class's `DefaultPrompt` constant (WorkflowEngine, the fallback actually used at runtime) and `DatabaseSeeder.BuiltInAgents` (Inference API, the seeded DB row). `VideoStoryEditorPromptConsistencyTests` asserts the two are byte-identical, so a prompt edit must always change BOTH sides in the same commit.
- **MassTransit** handles RabbitMQ messaging. Inference API publishes `WorkflowExecutionRequested`, WorkflowEngine consumes it.
- **Step Executors** implement `IStepExecutor` strategy pattern: `AgentStepExecutor`, `ConditionalStepExecutor`, `ForEachStepExecutor`, `ReviewLoopStepExecutor`, `ParallelStepExecutor`, `ExtractStepExecutor`, `VideoAnalyzeStepExecutor`, `VideoCompileStepExecutor`, and the three room executors `EditRoomStepExecutor`/`GraphicsRoomStepExecutor`/`ColorGradeRoomStepExecutor` (all thin bindings of the shared `RoomStepExecutorBase<TDecision>` — see `docs/video-editing.md` "The shared room infrastructure") (all four non-`Agent` deterministic types are non-LLM — no `IChatClient`/`IAgentRegistry` dependency, always retried at most once; the video pair also never throws, always emitting valid JSON even on failure, since `output_json` is `jsonb`).
...
- **Anthropic (Claude) provider kind** (`InferenceProviderKind.Anthropic`, official `Anthropic` NuGet SDK → `AsIChatClient`) serves `Chat` and `Vision` only — `Transcription` is rejected with a 400 on create/update/test since Anthropic ships no speech-to-text endpoint, with an explanatory `NotSupportedException` in `TranscriptionClientFactory` as a second line of defence. Deliberately a **model backend, not the [Claude Agent SDK](https://code.claude.com/docs/en/agent-sdk/overview)**: ReelForge's own harness (`ReelForgeAgentBase` + `AgentStepExecutor` + `ToolGroupCatalog` + the rooms) stays the harness, so the whole integration is one arm of `ChatClientFactory.Build` and no agent changed. The stored key selects its own transport — the literal `env:` sentinel defers to the SDK's own env/profile resolution (nothing persisted), `sk-ant-oat…` goes out as `Authorization: Bearer`, anything else as `x-api-key`; an **empty** key throws rather than silently falling back to the container's ambient `ANTHROPIC_API_KEY` (`ShouldAutoResolveCredentials` is get-only, and executing a workflow needs no admin rights, so a blank/undecryptable key would otherwise let any authenticated user spend the host's credential). The client is wrapped in `AnthropicChatOptionsAdapter`, which nulls `Temperature`/`TopP`/`TopK` (**every** agent sets a temperature, and Claude models after Opus 4.6 reject all three with a 400) plus the OpenAI-SDK-typed `RawRepresentationFactory` that carries `ReasoningEffort` — cloning, never mutating the singleton agent's reused `ChatOptions`, leaving both OpenAI paths byte-identical. Per-agent temperature and reasoning effort are consequently not applied on the Anthropic path; thinking is on at the model's default effort under `thinking.type=adaptive`, and those tokens count against `max_tokens`. **Credential boundary:** a Pro/Max subscription token is for individual use by its owner only — a deployed ReelForge serving other people must use a Console API key or Bedrock/Vertex/Foundry. **Dependency set (load-bearing):** `Microsoft.Agents.AI.*` (1.22.0), `Microsoft.Extensions.AI.*` (10.10.0, transitive), `OpenAI` (2.13.0), `Azure.AI.OpenAI` (2.9.0-beta.1) and `Anthropic` (12.49.0) move **together**. Versions compiled against different `Microsoft.Extensions.AI.Abstractions` releases link fine and throw `TypeLoadException` on the first agent run: Abstractions 10.4.0 removed seven public types that `Microsoft.Agents.AI.Workflows` 1.0.0-rc2 referenced from `Specialized.AIAgentHostExecutor`, which nothing here names but `InProcessExecution.RunStreamingAsync` instantiates to host an `AIAgent` — i.e. every room step. That took out all 13 room tests with `dotnet build` green, and only CI caught it. Before bumping any package in this set, scan the candidate assembly's type and member references against what the others ship (method in the `ReelForge.Shared.csproj` comment). `Azure.AI.OpenAI` is the laggard and the likeliest next blocker. See [`docs/anthropic-provider.md`](docs/anthropic-provider.md).
- **Step-result caching** (`WorkflowEngine/Execution/Caching`, `workflow_step_cache_entries`) lets a REPEATED execution reuse a prior successful step's output instead of re-calling the agent or re-running ffmpeg. The key is a SHA-256 over the project, step/agent identity, the agent's system prompt + output schema + provider + skills, every step config blob, the fully **resolved step input**, the user request, and a **project-file fingerprint**. The resolved input transitively contains all upstream output, so the key self-invalidates on any upstream change; the fingerprint covers what `ToolGroup.ProjectRead` agents read through tools rather than through the prompt. `StepCachePolicy` decides cacheability: `Extract`/`VideoAnalyze`/`VideoCompile`/`EditRoom`/`ColorGradeRoom` and read-only `Agent` steps yes; any `Agent` step whose grant includes `ProjectWrite`/`SandboxAuthoring`/`SandboxRender` **never** (those mutate state OUTSIDE the cached output — project files, sandbox workspace, a rendered object — that downstream steps depend on), `GraphicsRoom` never (its director may render an asset), `ReviewLoop` never (its purpose is to re-judge). `WorkflowStep.CacheMode` (`Default`/`Always`/`Never`) overrides per step. Hits carrying a storage key are HEAD-validated against MinIO first; every cache failure degrades to "no cache" and can never fail a workflow. A hit takes the identical persistence/publish path as a real result and is marked `WorkflowStepResult.FromCache` + `CachedTokensSaved`. See [`docs/workflow-execution-optimization.md`](docs/workflow-execution-optimization.md).
- **Prompt-context budget** (`WorkflowEngine/Execution/Context`) bounds how much prior-step output `BuildFullWorkflowInput`/`BuildSelectedPriorStepsInput` concatenate into an agent's prompt, which was previously unbounded and grew quadratically across a pipeline. The most recent `RecentStepsVerbatim` steps stay verbatim, older ones are compacted by `JsonOutputDigest` (a structure-preserving rewrite — long strings truncated, arrays capped, still valid JSON), then oldest-first dropping with an explicit marker. **Prompt-only**, exactly like the existing `SetPromptOutputOverride` contract: `StepOutputHistory`, the persisted `OutputJson`, and every deterministic by-`StepOrder` consumer keep seeing raw output. `AgentInputContextMode.PreviousStepOnly` and `CustomMappedSubset` are structurally exempt (the previous step's output is routinely a machine-consumed `{view, meta}` contract), and under-budget input is byte-identical to the previous behaviour. See [`docs/workflow-execution-optimization.md`](docs/workflow-execution-optimization.md).
...
| `WorkflowStepChatTurn` | WorkflowEngine | (available for consumers — Go API/frontend relay is a separate, follow-up pass) — append-only, sequence-numbered event published once per completed turn in a room step's (`StepType.EditRoom`/`GraphicsRoom`/`ColorGradeRoom`) group-chat run (structural twin of `WorkflowStepReasoningCaptured`, NOT a reuse of the ephemeral/supersedable `WorkflowStepProgress`); free-form model prose, never authoritative — see `docs/video-editing.md` "The edit room" |

### Inference Provider Endpoints (Inference API)

Admin-only CRUD for configured chat-completion providers (Azure OpenAI, an OpenAI-compatible
endpoint such as vLLM, or Anthropic), plus the per-agent override. Deliberately routed under
...
"The graphics room".
An eighth, opt-in template — `video-derush-edit-grade-room` (`AutoCreateOnProject: false`) —
adds a multi-agent "color grade room" to the derush pipeline: `VideoAnalyze`
(`Source: ProjectFile` — the default `AnalyzeVisuals: true` already emits the measured per-shot
colour words the room argues from) → `Agent(VideoStoryEditor)` → `ColorGradeRoom`
(`View: Step 1` — explicit, since `Previous` would resolve to the story editor's decision, not
the measured-shot envelope) → `VideoCompile` (`Decision: Step 2`, `enableColorGrade: true`,
`colorGradePlan: Step 3`, `AnalysisStepOrder: 1`) → `ReviewLoop(VideoReviewAgent)` looping back
to step 2. The `ColorGradeRoom` step emits the exact same `ColorGradePlanOutput` shape a solo
`Agent(Colorist)` step would, so the compile step consumes either interchangeably. See
[`docs/video-editing.md`](docs/video-editing.md) "The color grade room".
A ninth, opt-in template — `video-derush-edit-sfx` (`AutoCreateOnProject: false`) — extends
`video-derush-edit` with discrete sound effects: `VideoAnalyze` (`offerSfxClips: true`) →
`Agent(VideoStoryEditor)` → `Agent(SoundDesigner)` → `VideoCompile` (`enableSfx: true`,
`sfxPlan: Step 3`) → `ReviewLoop(VideoReviewAgent)` looping back to step 2. Same
explicit-`StepOrder` rationale as `video-derush-edit-music` (`Previous` relative to the compile
step would resolve to the `SoundDesigner` step's own output, not the story editor's decision).
See [`docs/video-editing.md`](docs/video-editing.md) "Sound effects".

### Video Editing (ffmpeg-based, `VideoAnalyze`/`VideoCompile`/`EditRoom`/`GraphicsRoom`/`ColorGradeRoom` step types)
...
- **`StepType.ColorGradeRoom`** (`Shared/Workflows/ColorGradeRoomStepConfig.cs`,
  `WorkflowEngine/Execution/StepExecutors/ColorGradeRoomStepExecutor.cs`) — the colour-grading
  analogue of `EditRoom`/`GraphicsRoom`, on the same shared room infrastructure: several
  `Colorist`-role seats (`ToneArtist`/`PaletteArtist`/`ContinuityArtist` by default) plus
  `AgentType.ColorGradeDirector` deliberate over a bounded `VideoAnalyze` view's measured
  per-shot colour facts (the `s{n}` SHOT ids anchor the deliberation and its convergence
  detection ONLY — the synthesized `ColorGradePlanOutput` carries no ids at all, one
  whole-program grade in enum words, so `FilterToOfferedIds` is a documented structural no-op
  and word validation replaces id filtering: an empty `Look` is retried, an unknown one spends
  one retry then defers to compile's `unknown_look_word` degrade). A `Look` of `"None"` is a
  fully valid outcome, and a zero-shot view completes with a `None` plan instead of failing.
  The compile step's `ColorGradePlan` (`EnableColorGrade`, default `false` — byte-identical off,
  requires `Mode = Reencode`) resolves the plan's words to a first-party
  `eq`/`colorbalance`/`colorlevels`/`hue` chain (`ColorGradeFilterBuilder`, exact-string-tested;
  internal tables, deliberately not config knobs) applied BEFORE overlays/inserts so graphics
  paint clean on graded footage; works on both encode paths (multi-source included). Every
  plan-level failure degrades to "no grade applied". Falls back to one ordinary solo
  `AgentType.Colorist` call (`FallbackToSoloColorist`, default `true`). See
  `docs/video-editing.md` § "Color grading" / "The color grade room".
...
| `ANTHROPIC_API_KEY` | — | Read by the Anthropic SDK itself (no ReelForge config binding) when an `inference_providers` row of kind `Anthropic` stores the literal `env:` as its key, so no secret is persisted. Injected into both `inference` and `workflow-engine`. See [`docs/anthropic-provider.md`](docs/anthropic-provider.md) |
| `ANTHROPIC_AUTH_TOKEN` | — | Same as above but sent as `Authorization: Bearer` — the OAuth/subscription shape. **A Pro/Max subscription token is licensed for ordinary, individual use by its owner**; a deployed ReelForge serving other people must use a Console API key instead |
...
| `WorkflowEngine:StepCache:Enabled` / `:TtlHours` / `:MaxEntryChars` / `:ValidateStorageArtifacts` | Cross-execution step-result cache (Engine only); defaults `true`/`168`/`2000000`/`true`. Omitting the section leaves the cache on at its code defaults — set `Enabled: false` to actually disable it |
| `WorkflowEngine:ContextBudget:Enabled` / `:MaxInputChars` / `:MaxDigestedStepChars` / `:RecentStepsVerbatim` / `:DigestMaxStringChars` / `:DigestMaxArrayItems` / `:DigestMaxDepth` | Prompt-context budget (Engine only); defaults `true`/`160000`/`12000`/`2`/`600`/`25`/`12`. `MaxInputChars <= 0` disables the overall cap |
