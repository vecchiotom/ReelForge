...
| `WorkflowStepChatTurn` | WorkflowEngine | (available for consumers — Go API/frontend relay is a separate, follow-up pass) — append-only, sequence-numbered event published once per completed turn in a `StepType.EditRoom` group-chat run (structural twin of `WorkflowStepReasoningCaptured`, NOT a reuse of the ephemeral/supersedable `WorkflowStepProgress`); free-form model prose, never authoritative — see `docs/video-editing.md` "The edit room" |
...
### Video Editing (ffmpeg-based, `VideoAnalyze`/`VideoCompile`/`EditRoom` step types)
