...
public abstract class RoomStepExecutorBase<TDecision> : IStepExecutor where TDecision : class
{
    ...
    /// <summary>Drops every id-bearing entry not in <paramref name="offeredIds"/>. Returns how many entries were dropped.</summary>
    protected abstract int FilterToOfferedIds(TDecision decision, HashSet<string> offeredIds);
        ...
        try
        {
            ...
            if (offeredIds.Count == 0)
            {
                ...
                return Success(context, sw, config, emptyViewDecision,
                    ...
                    synthesisAttempts: 0, droppedCount: 0,
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        droppedCount = FilterToOfferedIds(decision, offeredIds);
            ...
            if (degraded)
            {
                ...
                droppedCount = soloDropped;
            }

            return Success(context, sw, config, decision!,
                observedTurnCount, terminationReason, synthesisAttempts, droppedCount,
    ...
    private StepExecutionResult Success(
        ...
        int observedTurnCount, string terminationReason, int synthesisAttempts, int droppedCount,
    ...
    private async Task<(TDecision? Decision, int DroppedCount, string? Error)> RunSoloFallbackAsync(
        StepExecutionContext context, string viewJson, HashSet<string> offeredIds)
    {
        ...
        if (soloAgent is null)
            return (null, 0, $"AgentType.{SoloFallbackAgentType} is not registered for the solo fallback.");
        ...
        if (!result.Success)
            return (null, 0, result.FailureReason ?? "The solo fallback agent invoked FailWorkflow.");
